...
from sources.readers.procwind.provider import check_process_existence
from sources.screens.mainwind.interface import Ui_mainwind
from sources.screens.mainwind.worker import Worker
from sources.screens.procwind.operations import ProcWind
from sources.widgets.lgptwdgt.operations import LgPtWdgt
from sources.widgets.ntwkwdgt.operations import NtwkWdgt
from sources.widgets.perfwdgt.operations import PerfWdgt
from sources.widgets.phptwdgt.operations import PhPtWdgt
...
class MainWind(QMainWindow, Ui_mainwind):
    ...
    def initialize_activities_tabscreen(self):
        ...
        self.cntbvers.setText("System Resource Monitor")
    ...
    def refresh_partitions_tabscreen(self, statdict):
        ...
        try:
            ...
            logging.warning("Please click on the REFRESH button to obtain an updated list")
            self.statusBar.showMessage(
                "[%s] A physical partition was just added or removed - Please click on the REFRESH button to obtain an updated list"  
                % datetime.fromtimestamp(time.time()).strftime("%H:%M:%S")
            )
        try:
            ...
            logging.warning("Please click on the REFRESH button to obtain an updated list")
            self.statusBar.showMessage(
                "[%s] A logical partition was just added or removed - Please click on the REFRESH button to obtain an updated list"  
                % datetime.fromtimestamp(time.time()).strftime("%H:%M:%S")
            )
